            ...
            <div id="content-wrapper" class="d-flex flex-column">
                ...
                <div id="content">
                    ...
                    <div class="container-fluid">
                        ...
                        <!-- Page Heading -->
                        <h1 class="h3 mb-2 text-gray-800">RESOLUTIONS</h1>
                        ...
                        <?php
                            ...
                            $resolutions = $repo->getByType('résolution');

                            // Recherche par titre
                            if ($search !== '') {
                                $resolutions = array_filter($resolutions, function($doc) use ($search) {
                            ...
                            // Filtre catégorie
                            if ($categorieFilter) {
                                $resolutions = array_filter($resolutions, function($doc) use ($categorieFilter) {
                            ...
                            // Tri alphabétique
                            if ($alphaOrder) {
                                usort($resolutions, function($a, $b) use ($alphaOrder) {
                                    if ($alphaOrder === 'asc') {
                            ...
                            // Tri chronologique
                            if ($dateOrder) {
                                usort($resolutions, function($a, $b) use ($dateOrder) {
                                    if ($dateOrder === 'asc') {
                            ...
                            $resolutions = array_slice($resolutions, ($page - 1) * $perPage, $perPage);
                        ?>
                        ...
                        <?php
                            if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'])) {
                                ...
                                header("Location: listeRésolutions.php?success=1");
                                exit;
                            } 
                        ?> 
                        ...
                        <div class="card shadow mb-4" style="border: none; background: linear-gradient(135deg, #2563eb 0%, #60a5fa 100%); color: #fff;">
                            <div class="card-header py-3" style="background: #1e3a8a;">
                                <h6 class="m-0 font-weight-bold" style="color: #fff;">Liste des Résolutions</h6>
                                        ...
                                        <tbody>
                                            <?php if (count($resolutions) > 0): ?>
                                                <?php foreach ($resolutions as $resolution): ?>
                                                    <tr>
                                                        <td><?= htmlspecialchars($resolution['titre']) ?></td>
                                                        <td><?= htmlspecialchars($resolution['type']) ?></td>
                                                        <td><?= htmlspecialchars($resolution['categorie']) ?></td>
                                                        <td><?= htmlspecialchars(string: $resolution['description']) ?></td>
                                                        <td><?= date('d/m/Y H:i', strtotime($resolution['add_date'])) ?></td>
                                                        <td>
                                                            <?php if (!empty($resolution['url_fichier'])): ?>
                                                                <a href="controller/openDocument.php?id=<?= $resolution['id'] ?>" target="_blank" class="btn btn-sm btn-info"><i class="fas fa-folder-open"></i> </a>
                                                            <?php else: ?>
                                                                <span class="text-muted">Fichier non disponible</span>
                                                            <?php endif; ?>
                                                            <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'admin'): ?>
                                                                <a href="documentHistory.php?id=<?= $resolution['id'] ?>" class="btn btn-sm btn-info" title="Historique des actions"><i class="fas fa-history"></i></a>
                                                                <a href="controller/editRésolutions.php?id=<?= $resolution['id'] ?>" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i> </a>
                                                                <a href="controller/deleteRésolutions.php?id=<?= $resolution['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Supprimer ce document ?');"><i class="fas fa-trash"></i></a>
                                                            <?php endif; ?>
                                                        </td>
                                                    </tr>
                                                <?php endforeach; ?>
                                            <?php else: ?>
                                                <tr>
                                                    <td colspan="6" class="text-center">Aucune Résolution trouvée.</td>
                                                </tr>
                                            <?php endif; ?>
                                        </tbody>
                                    </table>
                    ...
                    <div class="modal-dialog modal-lg" role="document" >
                        <div class="modal-content">
                            ...
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Ajouter une Résolution</h5>
                                <button class="close" type="button" data-dismiss="modal" aria-label="Fermer">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            
                            <form action="controller/addRésolutions.php" method="POST" enctype="multipart/form-data" id="formAddDocument">
                                <div class="modal-body">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="docType" name="type" required>
                                            <option value="résolution" selected>Résolution</option>

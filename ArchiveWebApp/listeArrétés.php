            ...
            <div id="content-wrapper" class="d-flex flex-column">
                ...
                <div id="content">
                    ...
                    <div class="container-fluid">
                        ...
                        <!-- Page Heading -->
                        <h1 class="h3 mb-2 text-gray-800">ARRÊTÉS</h1>
                        ...
                        <?php
                            ...
                            $arretes = $repo->getByType('arrêtés');

                            // Recherche par titre
                            if ($search !== '') {
                                $arretes = array_filter($arretes, function($doc) use ($search) {
                            ...
                            // Filtre catégorie
                            if ($categorieFilter) {
                                $arretes = array_filter($arretes, function($doc) use ($categorieFilter) {
                            ...
                            // Tri alphabétique
                            if ($alphaOrder) {
                                usort($arretes, function($a, $b) use ($alphaOrder) {
                                    if ($alphaOrder === 'asc') {
                            ...
                            // Tri chronologique
                            if ($dateOrder) {
                                usort($arretes, function($a, $b) use ($dateOrder) {
                                    if ($dateOrder === 'asc') {
                            ...
                            $arretes = array_slice($arretes, ($page - 1) * $perPage, $perPage);
                        ?>
                            ...
                            <div class="card-body" style="background: #2563eb;">
                                <div class="table-responsive">
                                    <table class="table table-bordered" style="background: #eff6ff; color: #1e3a8a;">
                                        ...
                                        <tbody>
                                            <?php if (count($arretes) > 0): ?>
                                                <?php foreach ($arretes as $arrete): ?>
                                                    <tr>
                                                        <td><?= htmlspecialchars($arrete['titre']) ?></td>
                                                        <td><?= htmlspecialchars($arrete['type']) ?></td>
                                                        <td><?= htmlspecialchars($arrete['categorie']) ?></td>
                                                        <td><?= htmlspecialchars($arrete['description']) ?></td>
                                                        <td><?= date('d/m/Y H:i', strtotime($arrete['add_date'])) ?></td>
                                                        <td>
                                                            <?php if (!empty($arrete['url_fichier'])): ?>
                                                                <a href="controller/openDocument.php?id=<?= $arrete['id'] ?>" target="_blank" class="btn btn-sm btn-info"><i class="fas fa-folder-open"></i></a>
                                                            <?php else: ?>
                                                                <span class="text-muted">Fichier non disponible</span>
                                                            <?php endif; ?>
                                                            <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'admin'): ?>
                                                                <a href="documentHistory.php?id=<?= $arrete['id'] ?>" class="btn btn-sm btn-info" title="Historique des actions"><i class="fas fa-history"></i></a>
                                                                <a href="controller/editArrétés.php?id=<?= $arrete['id'] ?>" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                                                                <a href="controller/deleteArrétés.php?id=<?= $arrete['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Supprimer ce document ?');"><i class="fas fa-trash"></i></a>
                                                            <?php endif; ?>
                                                        </td>
                                                    </tr>
                                                <?php endforeach; ?>
                                            <?php else: ?>
                                                <tr>
                                                    <td colspan="6" class="text-center">Aucun arrêté trouvé.</td>
                                                </tr>
                                            <?php endif; ?>
                                        </tbody>
                                    </table>
                    ...
                    <div class="modal-dialog modal-lg" role="document" >
                        <div class="modal-content">
                            ...
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Ajouter un Arrêté</h5>
                                <button class="close" type="button" data-dismiss="modal" aria-label="Fermer">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            
                            <form action="controller/addArrétés.php" method="POST" enctype="multipart/form-data" id="formAddDocument">
                                <div class="modal-body">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="docType" name="type" required>
                                            <option value="arrêtés" selected>Arrêtés</option>

            ...
            <div id="content-wrapper" class="d-flex flex-column">
                ...
                <div id="content">
                    ...
                    <div class="container-fluid">
                        ...
                        <!-- Page Heading -->
                        <h1 class="h3 mb-2 text-gray-800">ORDONNANCES</h1>
                        <div class="d-sm-flex align-items-center justify-content-space-between mb-4" >
                            <a class="btn btn-sm btn-dark fw-bold" href="#modalAddOrdonnance" data-toggle="modal" data-target="#modalAddOrdonnance" style="margin-left: 0px">
                        ...
                        <?php
                            ...
                            $ordonnances = $repo->getByType('ordonnance');

                            // Recherche par titre
                            if ($search !== '') {
                                $ordonnances = array_filter($ordonnances, function($doc) use ($search) {
                            ...
                            // Filtre catégorie
                            if ($categorieFilter) {
                                $ordonnances = array_filter($ordonnances, function($doc) use ($categorieFilter) {
                            ...
                            // Tri alphabétique
                            if ($alphaOrder) {
                                usort($ordonnances, function($a, $b) use ($alphaOrder) {
                                    if ($alphaOrder === 'asc') {
                            ...
                            // Tri chronologique
                            if ($dateOrder) {
                                usort($ordonnances, function($a, $b) use ($dateOrder) {
                                    if ($dateOrder === 'asc') {
                            ...
                            $ordonnances = array_slice($ordonnances, ($page - 1) * $perPage, $perPage);
                        ?>
                        ...
                        <div class="card shadow mb-4" style="border: none; background: linear-gradient(135deg, #2563eb 0%, #60a5fa 100%); color: #fff;">
                            <div class="card-header py-3" style="background: #1e3a8a;">
                                <h6 class="m-0 font-weight-bold" style="color: #fff;">Liste des Ordonnances</h6>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered" style="background: #eff6ff; color: #1e3a8a;">
                                        ...
                                        <tbody>
                                            <?php if (count($ordonnances) > 0): ?>
                                                <?php foreach ($ordonnances as $ordonnance): ?>
                                                    <tr>
                                                        ...
                                                        <td><?= htmlspecialchars($ordonnance['type']) ?></td>
                                                        <td><?= htmlspecialchars($ordonnance['categorie']) ?></td>
                                                        <td><?= htmlspecialchars($ordonnance['description']) ?></td>
                                                        <td><?= date('d/m/Y H:i', strtotime($ordonnance['add_date'])) ?></td>
                                                        <td>
                                                             <?php if (!empty($ordonnance['url_fichier'])): ?>
                                                             <a href="controller/openDocument.php?id=<?= $ordonnance['id'] ?>" target="_blank" class="btn btn-sm btn-info"><i class="fas fa-folder-open"></i></a>
                                                             <?php else: ?>
                                                             <span class="text-muted">Fichier non disponible</span>
                                                            <?php endif; ?>
                                                            <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'admin'): ?>
                                                                <a href="documentHistory.php?id=<?= $ordonnance['id'] ?>" class="btn btn-sm btn-info" title="Historique des actions"><i class="fas fa-history"></i></a>
                                                                <a href="controller/editOrdonnance.php?id=<?= $ordonnance['id'] ?>" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                                                                <a href="controller/deleteOrdonnances.php?id=<?= $ordonnance['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Supprimer ce document ?');"><i class="fas fa-trash"></i></a>
                                                            <?php endif; ?>
                                                        </td>
                                                    </tr>
                                                <?php endforeach; ?>
                                            <?php else: ?>
                                                <tr>
                                                    <td colspan="6" class="text-center">Aucune ordonnance trouvée.</td>
                                                </tr>
                                            <?php endif; ?>
                                        </tbody>
                                    </table>
                ...
                <!-- Modal add Ordonnance -->
                <div class="modal fade" id="modalAddOrdonnance" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
                    <div class="modal-dialog modal-lg" role="document" >
                        <div class="modal-content">
                            ...
                            <form action="controller/addOrdonnance.php" method="POST" enctype="multipart/form-data" id="formAddDocument">
                                <div class="modal-body">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="docType" name="type" required>
                                            <option value="ordonnance" selected>Ordonnance</option>

        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-2 text-gray-800">DÉCISIONS</h1>
                    <div class="d-sm-flex align-items-center justify-content-space-between mb-4" >
                        <a class="btn btn-sm btn-dark fw-bold" href="#modalAddDecision" data-toggle="modal" data-target="#modalAddDecision" style="margin-left: 0px">
                    ...
                    <?php
                        ...
                        $decisions = $repo->getByType('décision');

                        // Recherche par titre
                        if ($search !== '') {
                            $decisions = array_filter($decisions, function($doc) use ($search) {
                        ...
                        // Filtre catégorie
                        if ($categorieFilter) {
                            $decisions = array_filter($decisions, function($doc) use ($categorieFilter) {
                        ...
                        // Tri alphabétique
                        if ($alphaOrder) {
                            usort($decisions, function($a, $b) use ($alphaOrder) {
                                if ($alphaOrder === 'asc') {
                        ...
                        // Tri chronologique
                        if ($dateOrder) {
                            usort($decisions, function($a, $b) use ($dateOrder) {
                                if ($dateOrder === 'asc') {
                        ...
                        $decisions = array_slice($decisions, ($page - 1) * $perPage, $perPage);
                    ?>
                    ...
                    <div class="card shadow mb-4" style="border: none; background: linear-gradient(135deg, #2563eb 0%, #60a5fa 100%); color: #fff;">
                        <div class="card-header py-3" style="background: #1e3a8a;">
                            <h6 class="m-0 font-weight-bold" style="color: #fff;">Liste des Décisions</h6>
                                    ...
                                    <tbody>
                                        <?php if (count($decisions) > 0): ?>
                                            <?php foreach ($decisions as $decision): ?>
                                                <tr>
                                                    <td><?= htmlspecialchars($decision['titre']) ?></td>
                                                    <td><?= htmlspecialchars($decision['type']) ?></td>
                                                    <td><?= htmlspecialchars($decision['categorie']) ?></td>
                                                    <td><?= htmlspecialchars(string: $decision['description']) ?></td>
                                                    <td><?= date('d/m/Y H:i', strtotime($decision['add_date'])) ?></td>
                                                    <td>
                                                        <?php if (!empty($decision['url_fichier'])): ?>
                                                            <a href="controller/openDocument.php?id=<?= $decision['id'] ?>" target="_blank" class="btn btn-sm btn-info"><i class="fas fa-folder-open"></i></a>
                                                        <?php else: ?>
                                                            <span class="text-muted">Fichier non disponible</span>
                                                        <?php endif; ?>
                                                        <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'admin'): ?>
                                                            <a href="documentHistory.php?id=<?= $decision['id'] ?>" class="btn btn-sm btn-info" title="Historique des actions"><i class="fas fa-history"></i></a>
                                                            <a href="controller/editDécisions.php?id=<?= $decision['id'] ?>" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                                                            <a href="controller/deleteDécisions.php?id=<?= $decision['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Supprimer ce document ?');"><i class="fas fa-trash"></i></a>
                                                        <?php endif; ?>
                                                    </td>
                                                </tr>
                                            <?php endforeach; ?>
                                        <?php else: ?>
                                            <tr>
                                                <td colspan="6" class="text-center">Aucune décision trouvée.</td>
                                            </tr>
                                        <?php endif; ?>
                                    </tbody>
                                </table>
            ...
            <!-- Modal add Décision -->
            <div class="modal fade" id="modalAddDecision" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
                <div class="modal-dialog modal-lg" role="document" >
                    <div class="modal-content">
                        
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Ajouter une Décision</h5>
                            <button class="close" type="button" data-dismiss="modal" aria-label="Fermer">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        
                        <form action="controller/addDécisions.php" method="POST" enctype="multipart/form-data" id="formAddDocument">
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="docType" name="type" required>
                                        <option value="décision" selected>Décision</option>
